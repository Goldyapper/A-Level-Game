...
screen = pygame.display.set_mode(Window_Size,32)#initiates the window
...
spikes_img.set_colorkey((255,255,255))#white pixels of the player are transparent
...
game_map=load_map('Map_1-4')#reads the file for the map

#########################################################################################
                                ###Background Objects Part 1###
#########################################################################################

background_objects = [[0.25,[120,10,70,400]],[0.25,[280,30,40,400]],[0.5,[30,40,40,400]],[0.5,[130,90,100,400]],[0.5,[300,50,120,400]]]# first number is times by how far the camera has moved to make the effect that objects are moving in the background, rest of the numbers are the bas data for the objects
...
#########################################################################################
    ...
    for layer in game_map:
        ...
        for tile in layer:
            ...
            if tile == '5':
                display.blit(spikes_img,(x*16,y*16))#draw the platform if 1 and times by the pixel size
                tile_rects.append(pygame.Rect(x*16,y*16,16,16))#the platforms hit box, the second pair being how big the hitbox is
            x += 1
        y += 1
