...
font_2 = pygame.font.SysFont('Courier',128)###load images###
    ...
    if difficulty_menu == True:#if the difficulty menu is being displayed
        ...
        if game_over == 1:#if player has completed the level
            ...
            if level <= max_level:#if is not the last level of the set
                ...
                c_seconds = int((centiseconds)*100)#converts centiseconds into a 2 digit interger


                draw_text(f'1{minutes}:1{seconds}:{c_seconds}',font_2,white,16,tile_size)#displays the time passed on screen, when their is 2 digits in each section, it is in the centre of the screen
