...
max_level = 4 #max level variable
SPX = 100 #spawn point on the x-axis
SPY = (Screen_height-136)#spawn point on the y-axis
...
def reset_level(level):
    ...
    ###make new world map###
    if path.exists(f'Maps/Set_1/level{level}_data'):#checks if the level data exists
        pickle_in = open(f'Maps/Set_1/level{level}_data','rb')#f formats the stirng, curly brackets adds the level variable to the stirng
...
class Player(): #code for player
    ...
    def update(self,game_over):#updates player
        ...
        if game_over == 0:#only happens if game_over = 0
            ...
            if pygame.sprite.spritecollide(self,exit_group, False):#if spike sprites collide with the player
...
class World(): #code for world
    ...
    def __init__(self,data): #init takes a self and data argument
        ...
        ###loop that reads map and decides whata to display###
        y = 0 #y co-ords
        for layer in data:
            x = 0 # x co-ords
            for tile in layer:
                ...
                if tile == 4:
                    exit = Exit(x * tile_size,y * tile_size - (tile_size))#createas a spike instance and positions it at the x and y co-ords
                    exit_group.add(exit)#adds spikes to the spikes group
...
exit_group = pygame.sprite.Group()#creats thee exit group
