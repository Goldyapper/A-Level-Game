...
font = pygame.font.SysFont('Arial', 32)#imprts the font arial and the size of 32 to be used later
...
### Game Variables ###
lives = 5# lives variable ###to be changed when i add the difficulty menu###
tile_size = 32 #size of each tile 
game_over = 0 #game over variable
max_level = 5 #max level variable
level = 3#level varaible 
Set = 3#set of level variable
...
###load images###
restart_level_img = pygame.image.load('images/restart_level.png')
restart_set_img = pygame.image.load('images/restart_set.png')
...
class Player(): #code for player
    def __init__(self,x,y):#takes arguement self,x coords, y coords
        ...
        self.lives = lives#sets self.lives to the lives variable
    ...
    def update(self,game_over):#updates player
        ...
        if game_over == 0:#only happens if game_over = 0
            ...
            if pygame.sprite.spritecollide(self,spikes_group, False):#if spike sprites collide with the player
                player.lives -= 1#player has died
                game_over = -1
...
###buttons###
restart_level_button = Button(Screen_width//2 -30, Screen_height//2, restart_level_img)
restart_set_button = Button(Screen_width//2 -30, Screen_height//2, restart_set_img)
...
while run:#game loop
    ...
    left_spikes_group.draw(Screen)#draws the spikes onto the screen
    right_spikes_group.draw(Screen)#draws the spikes onto the screen
    print (player.lives)#always prints the lives of the player (testing)
    ...
    if game_over == -1:       
        if player.lives == 0:#player has run out of lives
            if restart_set_button.draw():#restart button is drawn
            ...
            if restart_level_button.draw():#draws button and returns the variable 'action'
                ###when player dies the level resets###
                world.data = []#clear world.data
                world = reset_level(level)#resets level with the arguement lece
                game_over = 0#game restarts
                coin_count = 0#player's score resets ####change with lives algorthm####
